...
|| p.sellerLastName.Contains(term)).ToList();
        ...
        {
            var list = db.AssociatedSharedSouthUnSold.Include(s => s.SharedId).Include(p => p.productId).Where(p => p.productId.ProductId == productId).Where(s => s.SharedId.Id == accountId).ToList();
            if ((list.Count != 0))
                return true;
            else
                return false;
