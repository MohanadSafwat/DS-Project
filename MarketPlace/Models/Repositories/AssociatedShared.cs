...
namespace MarketPlace.Models.Repositories
{
    public class AssociatedSharedRepository : IAssociatedRepository<AssociatedShared, ProductSharedReadDto>
    {
        ...
        {
            if ((db.AssociatedSharedUnSold.Where(p => p.productId.ProductId == productId).Where(s => s.SharedId.Id == accountId) != null) || (db.AssociatedSharedUnSold.Where(p => p.productId.ProductId == productId).Where(s => s.SharedId.Id == accountId)) != null)
                return true;
            else
                return false;
        }
